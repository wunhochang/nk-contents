...
{
    public class MoviePublictionInfo : Gentity
    {
      internal static void GetList(Hashtable condition, ref APIResult result)
      {
          try
          {
              ...
              {
                    var sql = DB.GetQuery("movie_publiction_info", "GetList", condition);
                    ...
                    {
                        sbInString.Append(" and a.movie_name like '%" + DBUtils.DataToString(condition["searchtxt"]) + "%' ");
                    }
        ...
        internal static void GetDetail(Hashtable condition, ref APIResult result)
        {
            try
            {
                var data = new Hashtable();
                using (var DB = Config.GetDatabase())
                {
                    var sql = DB.GetQuery("movie_publiction_info", "GetDetail", condition);
        ...
        internal static void GetComboList(Hashtable condition, ref APIResult result)
        {
            try
            {
                ...
                using (var DB = Config.GetDatabase())
                {
                    var sql = DB.GetQuery("movie_publiction_info", "GetComboList", condition);
