    ...
    public class SpCorp : Gentity
    {
      internal static void GetList(Hashtable condition, ref APIResult result)
      {
          try
          {
              ...
              {
                    var sql = DB.GetQuery("sp_corp", "GetList", condition);
                    ...
                    {
                        sbInString.Append(" and (a.sp_corp_name like '%" + DBUtils.DataToString(condition["searchtxt"]) + "%' ");
                        ...
                        /*
                        sbInString.Append(" or c.sp_corp_manager_name like '%" + DBUtils.DataToString(condition["searchtxt"]) + "%' ");
                        sbInString.Append(" or c.sp_corp_manager_tel1 like '%" + DBUtils.DataToString(condition["searchtxt"]) + "%' ");
                        sbInString.Append(" or c.sp_corp_manager_tel2 like '%" + DBUtils.DataToString(condition["searchtxt"]) + "%') ");
                        */
                    }
      ...
      internal static void GetReportComboList(Hashtable condition, ref APIResult result)
        {
            try
            {
                ...
                using (var DB = Config.GetDatabase())
                {
                    var sql = DB.GetQuery("sp_corp", "GetReportComboList", condition);
      ...
      internal static void GetComboList(Hashtable condition, ref APIResult result)
        {
            try
            {
                ...
                using (var DB = Config.GetDatabase())
                {
                    var sql = DB.GetQuery("sp_corp", "GetComboList", condition);
