...
{
    public class AccountsProfitLoss : Gentity
    {
      internal static void GetList(Hashtable condition, ref APIResult result)
      {
          try
          {
              ...
              using (var DB = Config.GetDatabase())
              {
                    ...
                    var sql = DB.GetQuery("accounts_profit_loss", "GetList", condition);
      ...
      internal static void GetMovieRawPrice(Hashtable condition, ref APIResult result)
        {
            try
            {
                var data = new Hashtable();
                using (var DB = Config.GetDatabase())
                {
                    var sql = DB.GetQuery("accounts_profit_loss", "GetMovieRawPrice", condition);
