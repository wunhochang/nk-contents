    ...
    public class Role : Gentity
    {
      internal static void GetList(Hashtable condition, ref APIResult result)
      {
          try
          {
              ...
              {
                    var sql = DB.GetQuery("role", "GetList", condition);
                    ...
                    {
                        sbInString.Append(" and (role_name like '%" + DBUtils.DataToString(condition["searchtxt"]) + "%' ");
                        sbInString.Append(" or role_code like '%" + DBUtils.DataToString(condition["searchtxt"]) + "%') ");
                    }
      ...
      internal static void GetComboList(Hashtable condition, ref APIResult result)
        {
            try
            {
                ...
                using (var DB = Config.GetDatabase())
                {
                    var sql = DB.GetQuery("role", "GetComboList", condition);
