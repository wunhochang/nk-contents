...
import CommonCodeViewModel from './CommonCodeViewModel';
Ext.require([
  'Ext.data.*',
  'Ext.grid.*',
  'Ext.util.*'
]);
export default class CommonCodeView extends Component {
  state = {
    isOpen:false,
    result:[],
  ...
  store = Ext.create('Ext.data.Store', {
    ...
    proxy: {
        ...
        url: `${API_URL}/CommonCode/GetList`,
        actionMethods: {
            read: "GET"
        },
        extraParams: {
        ...
        if(process_store.length == 0){
          ...
          Ext.each(process_store, function (itm) {
            if(itm.phantom == true || itm.dirty == true){
              if(itm.data.type_name ==''){
                errorCheck1++;
              }
              if(itm.data.type_code ==''){
                errorCheck2++;
              }
              if(itm.data.type_data ==''){
                errorCheck3++;
              }
              params.push(itm);
            }
          ...
          if(errorCheck1>0){
            alert('데이터 그룹명을 입력해주세요.');
            return;
          }
          if(errorCheck2>0){
            alert('데이터 코드를 입력해주세요.');
            return;
          }
          if(errorCheck3>0){
            alert('데이터명을 입력해주세요.');
  ...
  onClickDelete = (item) => {
    const me = this,
          ...
          showConfirm({
              msg: '공통코드 정보를 삭제하시겠습니까?',
              title:'확인',
              callback: function () {
                Ext.each(selection, function (itm) {
                  if(!(itm.data.common_code_no == 0 || itm.data.common_code_no == '' || itm.data.common_code_no ==  null)){
                    promiseList.push(new Ext.Promise(function (res) {
                      Ext.Ajax.request({
                          url: `${API_URL}/CommonCode/Delete/`+itm.data.common_code_no,
                          method: 'DELETE',
                          headers: {'Content-Type': 'application/json;charset=UTF-8;','Authorization':cookie.load('token') },
                          dataType: "json",
                          callback: function () {
           ...
           items={[{
             ...
             tbar:[{
               ...
               columns:[
                 ...
                 {
                     text: '<font color=red>*</font><font color=blue>데이터그룹명</font>',
                     dataIndex: 'type_name',
                     type: 'TEXT',
                     width: 140,
                     isExcel: true,
                     ...
                     renderer: function(value, metaData, record) {
                       var deviceDetail = record.get('type_name');
                 ...
                 {
                      text: '<font color=red>*</font><font color=blue>데이터코드</font>',
                      dataIndex: 'type_code',
                      type: 'TEXT',
                      width: 140,
                      isExcel: true,
                      ...
                      renderer: function(value, metaData, record) {
                        var deviceDetail = record.get('type_code');
                        metaData.tdAttr = 'data-qtip="' + deviceDetail + '"';
                        return value;
                      },
                      filter: {
                 ...
                 {
                     text: '<font color=red>*</font><font color=blue>테이터명</font>',
                     dataIndex: 'type_data',
                     type: 'TEXT',
                     width: 140,
                     isExcel: true,
                     ...
                     renderer: function(value, metaData, record) {
                       var deviceDetail = record.get('type_data');
                       metaData.tdAttr = 'data-qtip="' + deviceDetail + '"';
                       return value;
                     },
                     filter: {
                 ...
                 {
                     text: '<font color=blue>상위데이터그룹명</font>',
                 ...
                 {
                      text: '<font color=blue>상위데이터코드</font>',
                      dataIndex: 'parent_type_code',
                      type: 'TEXT',
                      width: 140,
                      isExcel: true,
                      align: 'left',
