...
export default class MenuSecondView extends Component {
  constructor(props) {
      super(props);
      this.secondMenuCall = this.secondMenuCall.bind(this);
  }
  secondMenuCall = (result) => {
    this.props.secondMenuCall(result);
  }
  state = {
    isOpen:false,
    result:[],
  ...
  componentDidMount(){
    ...
    //   form = second_grid.down('form'),
    //   grid = second_grid.down('grid'),
    //   store = grid.getStore(),
    //   formValue = {depth:1},
  ...
  mainDataStoreLoad = () => {
    const me = this,
      ...
      grid = second_grid.down('grid'),
      store = grid.getStore(),
      formValue = {depth:2, parent_menu_no:this.state.parent_menu_no};
      //formValue = form.getValues();
  ...
  onRowClick = (view, rec, node, rowIdx, eOpt, e) =>{
    const {second_grid} = this.refs,
          ...
          if(selection.length == 1){
            this.secondMenuCall(rec);
          }
          else{
            this.secondMenuCall(null);
          }
  }
