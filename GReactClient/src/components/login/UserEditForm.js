...
export default class RegisterForm extends Component {
  ...
  componentDidMount(){
    ...
    user_id.value = this.state.user.user_id;
    user_name.value = this.state.user.user_name;
    user_tel.value = this.state.user.user_tel;
    user_email.value = this.state.user.user_email;
    user_name.focus();
  }
  onUserEditClose = () => {
    this.props.onUserEditClose();
  }
  onKeyPressUserId = (e) => {
  ...
  fnRegister = () => {
    const {user_id, user_name, user_tel, user_email, user_pwd, user_re_pwd} = this.refs,
          me = this,
          params = paramsPostHandler({
                    user_no:this.state.user.user_no,
                    ...
                    <div className="col-md-6">
                        <div className="card mx-4">
                            <div className="card-block p-4">
                                <h2>개인정보수정</h2>{/*Register*/}
                                <p className="text-muted">사용자 정보를 아래 입력해주세요.</p>{/*Create your account*/}
                                ...
                                <button type="button" className="btn btn-block btn-success" onClick={this.onClickRegister}>정보수정</button>{/*Create Account*/}
                                <button type="button" className="btn btn-block btn-success-login" onClick={this.onUserEditClose}>닫기</button>{/*Create Account*/}
                            </div>
                        </div>
                    </div>
                </div>
