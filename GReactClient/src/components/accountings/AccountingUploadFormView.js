...
import {ExcelFileDownload} from '../../utils/Util';
...
export default class AccountingUploadFormView extends Component {
  state = {
    isOpen:false,
    sp_corp_detail_no : 0,
    sp_corp_detail_name : '',
    ...
    this.accounting_end_date_store.load({
        ...
        callback: function (recs, operation, success) {
          ...
          if(res!=null){
            ...
            if(typeof(ret.data) != 'undefined' && ret.data != null && ret.data.length>0){
  ...
  uploadSuccess = (res) => {
    ...
    if(res != null && res.data != null){
      for(var i=0; i<res.data.length; i++){
        if(formValues.accounting_type == 'A' || formValues.accounting_type == 'B' || formValues.accounting_type == 'E'){
          var itm = res.data[i],
              movie_no = 0,
              movie_name = '',
              movie_check = 'X',
              ...
              /*핑크영화*/
              if(formValues.accounting_type == 'B'){
                var ridx = me.sp_distribution_enterprise_type_store.findExact('type_data', itm.col_09);
                if(ridx == -1){
                  distribution_enterprise_type = '';
                  distribution_enterprise_type_name = '';
                }
                  ...
                  distribution_enterprise_type_name = itm.col_09;
                }
              }
              var movie_info = {
        ...
        else if(formValues.accounting_type == 'C'){
          var itm = res.data[i],
              movie_no = 0,
              movie_name = '',
              movie_check = 'X',
              ...
              distribution_enterprise_type='',
              distribution_enterprise_type_name='',
              recordIndex = me.grid_movie_tag_store.findExact('movie_tag_name', itm.col_02);
              if(recordIndex == -1){
                movie_name = '';
                movie_no = 0;
                movie_check = 'X';
              ...
              var movie_info = {
                ...
                accounting_type:formValues.accounting_type,
                sp_sales_date:'',
                sp_cor_accounting_date:itm.col_01,
                movie_tag_name:itm.col_02,
                up_sp_corp_name:'',
                sales_price:0,
                accounting_rates:0,
                accounting_price:itm.col_03,
                sales_kind:'',
                sales_kind_name:'',
                distribution_enterprise_type:'',
                distribution_enterprise_type_name:'',
                ...
                movie_no:movie_no,
                tag_flag:tag_flag,
                raw_nk_price:itm.col_04,
                raw_monitor_price:itm.col_05,
                raw_price:itm.col_06
              };
              me.grid_store.add(movie_info);
        }
      }
  ...
  basic_sample = () => {
    var url = `${API_URL}/AccountingSummary/SampleDownload`;
    ExcelFileDownload(url, 'basic_sample.xlsx');
  }
  pink_sample = () => {
    var url = `${API_URL}/AccountingSummary/SampleDownload`;
    ExcelFileDownload(url, 'pink_sample.xlsx');
  }
  raw_sample = () => {
    var url = `${API_URL}/AccountingSummary/SampleDownload`;
    ExcelFileDownload(url, 'raw_sample.xlsx');
  }
  onSave = () => {
    const me = this,
          {result, showDisplayRefresh} = this.props,
          {model, accounting_end_date} = this.state,
          {header_grid, reqform} = this.refs,
          form = reqform.down('form'),
          formValues = form.getValues(),
          grid = header_grid.down('grid'),
    ...
    var accounting_rawdata_list = [],
        ...
        accounting_end_date_count = 0,
        sales_kind_count = 0,
        distribution_enterprise_type_count = 0;
    if(store.count()>0){
        for(var i=0; i<store.count(); i++){
          ...
          if(i==0){
            ...
            if(parseFloat(accounting_end_date.replaceAll('-','')) >= parseFloat(settlement_date.replaceAll('-',''))){
              accounting_end_date_count++;
            }
          }
          //console.log(rec);
          ...
          if(formValues.accounting_type == 'A' || formValues.accounting_type == 'B' || formValues.accounting_type == 'D' || formValues.accounting_type == 'E'){
            if( typeof(rec.data.sales_kind) == 'undefined' || rec.data.sales_kind == '' || rec.data.sales_kind == null){
              sales_kind_count++;
            }
          }
          if(formValues.accounting_type == 'B'){
            if( typeof(rec.data.distribution_enterprise_type) == 'undefined' || rec.data.distribution_enterprise_type == '' || rec.data.distribution_enterprise_type == null){
              distribution_enterprise_type_count++;
            }
          }
          /*단매일경우*/
          if(formValues.accounting_type == 'D'){
            if(i==0){
        ...
        /*if(accounting_end_date_count>0){
          alert('최종정산마감연월은 <strong>' + accounting_end_date + '</strong> 입니다.<br><br>업로드 정산서 매출월을 다시 확인해주세요.');
          return;
        }*/
        ...
        if(formValues.accounting_type == 'A' || formValues.accounting_type == 'D'){
          if(sales_kind_count>0){
            alert('선택되지 않은 판권구분 정보가 있습니다. 다시 확인해주세요.');
            return;
          }
        }
        if(formValues.accounting_type == 'B'){
          if(distribution_enterprise_type_count>0){
            alert('선택되지 않은 유통업체 정보가 있습니다. 다시 확인해주세요.');
            return;
          }
        }
        /*정산 요약정보 생성*/
        ...
        callback: function () {
          Ext.Ajax.request({
              ...
              failure: function(response, request) {
                ...
                if(ret && ret.errors && ret.errors[0].message)
                  alert(ret.errors[0].message);
                else
                  alert("알수없는 오류입니다.");
              }
  ...
  onReconfigure = (accounting_type ) =>{
    ...
    if(accounting_type == 'D'){
      grid_model.push(
        ...
        // },
        {
            text: '<font color=red>*</font><font color=blue>영화명</font>',
            dataIndex: 'movie_no',
            type: 'COMBO',
            width: 300,
            isExcel: true,
                      ...
                      <Container
                                    ...
                                    items: [
                                      {
                                          ...
                                          items:[
                                          {
                                              ...
                                              listeners: {
                                                  change: this.onAccountingReportChange
                                              }
                                            },
                                            {
                                                xtype: 'combo',
                                                fieldLabel: '<font color=red>*</font>SP사 선택',
                                                name: 'sp_corp_detail_no',
                                                flex:1,
                                                queryMode: 'local',
                                                displayField: 'full_sp_corp_detail_name',
                                                valueField: 'sp_corp_detail_no',
                                                typeAhead: false,
                                                emptyText: '선택',
                                                editable: true,
                          ...
                          <Container
                              ...
                              <Container
                                ...
                                ref={'btn_file_upload'}>
                                <BtnFileUpload
                                  sp_corp_detail_no={this.state.sp_corp_detail_no}
                                  sp_corp_detail_name={this.state.sp_corp_detail_name}
                                  title={'양식첨부'}
                                  btn_width={80}
                              ...
                              <Button
                                  ...
                                  onClick={this.close}/>
                              <Button
                                  ref={'btn_basic_sample'}
                                  text={'기본DOWN'}
                                  hidden={false}
                                  style={{'font-size': '14px','background-color':'#4783AE','border-color':'#4783AE','padding':'5px 7px 4px 7px','border-radius': '4px 4px 4px 4px'}}
                                  margin={'2 5 0 20'}
                                  onClick={this.basic_sample}/>
                              <Button
                                  ref={'btn_pink_sample'}
                                  text={'핑크DOWN'}
                                  hidden={false}
