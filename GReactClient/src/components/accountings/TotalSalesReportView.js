...
export default class TotalSalesReportView extends Component {
  state = {
  ...
  store = Ext.create('Ext.data.Store', {
    ...
    proxy: {
        ...
        url: `${API_URL}/AccountingSummary/GetTotalSalesReport`,
        actionMethods: {
            read: "GET"
        },
        extraParams: {
          ...
          if(success == false && operation.error.status == '401'){
            ...
            if(res!=null){
              const ret = Ext.decode(res.responseText),
                    ...
                    counter.setHtml('<strong>총 ' + Ext.util.Format.number(totalCount, '0,000') + '건</strong> *** 데이터 변경시 익일 02시 반영');
         ...
         items={[{
          ...
          //  },
           ...
           tbar:[{
               xtype:'component',
               code: 'counter',
               html: '<strong>총: 0건</strong> *** 데이터 변경시 익일 02시 반영'
             },'->',
             {
               xtype:'button',
